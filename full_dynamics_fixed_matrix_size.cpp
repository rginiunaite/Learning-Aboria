...
int main() {
    ...
    const int n_filo = 2; // number of random directions
    ...
    for (int t = 0; t < N_steps; t++) {
        ...
        */
        ...
        for (int i = 0; i < particles.size(); i++) {
            ...
            std::array<double, n_filo> random_angle;
            std::array<int, n_filo> sign_x;
            std::array<int, n_filo> sign_y;
            ...
            for (int i = 0; i < n_filo; i++) {
                ...
                array<int, n_filo> new_chemo_search;

                for (int i =0; i< n_filo;i++){
                    new_chemo_search[i] = chemo(round((x[0] * (length_x / domain_length) + sin(random_angle[i]) + sign_x[i] * l_filo)));
                }
                ...
                double greater_conc;
                int index_greater_conc;
                if (new_chemo_search[0]>= new_chemo_search[1]){
                    greater_conc = new_chemo_search[0];
                    index_greater_conc = 0;
                }
                else{
                    greater_conc = new_chemo_search[1];
                    index_greater_conc =1;
                }

                // compare absolute difference

                //if (new_chemo-old_chemo > diff_conc){

                // compare relative difference

                if ((greater_conc-old_chemo)/sqrt(old_chemo) > diff_conc) {
                     ...
                     */
                    x += vdouble2(sin(random_angle[index_greater_conc]), cos(random_angle[index_greater_conc]));
                    ...
                    if (free_position == true && round((x[0] * (length_x / domain_length))) > 0 &&
                        ...
                        get<position>(particles)[i] += vdouble2(sin(random_angle[index_greater_conc]),
                                                                cos(random_angle[index_greater_conc])); // update if nothing is in the next position
                    }
